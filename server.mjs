...
const environment = existsSync(join(root, '.env'))
  ? Object.fromEntries(
...
const server = createServer((request, response) => {
  ...
  if (requestPath === '/runtime-config.js' && environment.VITE_SUPABASE_URL && environment.VITE_SUPABASE_PUBLISHABLE_KEY) {
    const config = { url: environment.VITE_SUPABASE_URL, publishableKey: environment.VITE_SUPABASE_PUBLISHABLE_KEY };
    response.writeHead(200, { 'Content-Type': 'text/javascript', 'Cache-Control': 'no-store' });
    response.end(`globalThis.__SUPABASE_CONFIG__ = ${JSON.stringify(config)};`);
    return;
  }
